...
- [SwapTitan](https://swaptitan.net) - No-KYC cross-chain crypto swap API for AI agents — 1288+ assets with automatic best-rate routing. **H2H Privacy Routing** pipes any-to-any swaps through Monero so source and destination chains stay unlinkable on-chain. 18 MCP tools: cross-chain `swap_quote`/`swap_create`/`swap_status` (with `minAmount` validation), Solana DEX swaps (Jupiter), EVM swaps on 7 chains (KyberSwap), Hyperliquid perps with builder codes, `rug_check`, portfolio and price tools. Pay-per-call via x402 (USDC on Base): $0.05 swap execution, $0.01 JSON-RPC gateway, $0.025 AI chat, $0.02 AI market-allocation oracle — or €49.99/mo Agent Tier via `X-License-Key`. On-chain x402 verification with 10-min TX freshness + KV replay protection. ([MCP Server](https://swaptitan.net/mcp)) ([Pricing](https://swaptitan.net/pricing)) ([GitHub](https://github.com/polsolbridge/solana-swap-api)) ([Agent Card](https://swaptitan.net/.well-known/agent.json))
